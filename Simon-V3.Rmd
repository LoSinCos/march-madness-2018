...
results <- read.csv("RegularSeasonCompactResults.csv")
results<-subset(results,Season==year)
testData<-read.csv("NCAATourneyCompactResults.csv")
...
```{r}
FinalPredictions$LogLoss<-testDataEX$result*log(FinalPredictions$Pred)+(1-testDataEX$result)*
  log(1-FinalPredictions$Pred)
head(FinalPredictions)
```

```{r}
GamesPlayed = dim(FinalPredictions)[1]
FinalLogLoss = sum(FinalPredictions$LogLoss)*(-1/GamesPlayed)
print(FinalLogLoss)
```
