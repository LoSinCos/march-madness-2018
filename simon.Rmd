...
WinLoss<-read.csv("data/allteams-allseason-allcoaches-winloss-1985-2017.csv")
...
Join Coach Win/Loss Data
```{r}

WinLoss


T1WinLoss<-select(WinLoss,c(1:2,17:19))
T2WinLoss<-select(WinLoss,c(1:2,17:19))

colnames(T1WinLoss)<-paste("T1",colnames(T1WinLoss),sep="_")
colnames(T2WinLoss)<-paste("T2",colnames(T2WinLoss),sep="_")

resultsEX %<>% left_join(., T1WinLoss, by = c("Season"="T1_Season", "Team1"="T1_TeamID"))
resultsEX %<>% left_join(., T2WinLoss, by = c("Season"="T2_Season", "Team2"="T2_TeamID"))

resultsEX %<>% mutate(Coach_Windiff = T1_CoachWinPer - T2_CoachWinPer)
resultsEX %<>% mutate(Coach_Expdiff = T1_CoachYears - T2_CoachYears)
...
testDataEX %<>% left_join(., T1WinLoss, by = c("Season"="T1_Season", "Team1"="T1_TeamID"))
testDataEX %<>% left_join(., T2WinLoss, by = c("Season"="T2_Season", "Team2"="T2_TeamID"))

testDataEX %<>% mutate(Coach_Windiff = T1_CoachWinPer - T2_CoachWinPer)
testDataEX %<>% mutate(Coach_Expdiff = T1_CoachYears - T2_CoachYears)
...
```{r}
logitModel<-glm(result~seed_diff+Coach_Windiff+Coach_Expdiff,data=resultsEX,family=binomial(link="logit"))
summary(logitModel)
```
